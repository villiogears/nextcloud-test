...
`MYSQL_ROOT_HOST` はデフォルトで `%`（任意ホスト）に設定してあり、コンテナのネットワーク範囲に依存せずにアプリケーションコンテナからの接続拒否 (`Host 'xxx' is not allowed to connect`) を防ぎます。セキュリティ上、本番や共有環境では `172.%` や `%.nextcloud-test_nextcloud` などネットワークを限定した値に必ず調整してください。
