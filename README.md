...
`MYSQL_ROOT_HOST` はデフォルトで `%`（任意ホスト）に設定してあり、コンテナのネットワーク範囲に依存せずにアプリケーションコンテナからの接続拒否 (`Host 'xxx' is not allowed to connect`) を防ぎます。環境に合わせて必要であればより限定的な CIDR/ワイルドカードに調整してください。
